...
const ReorderIndex = () => {
    ...
    const onReorder = async (index: number) => {
      ...
      const res1 = await api.patch("/careers", {...dataUpdated[index], timestamp: oldData.timestamp});
      const res2 = await api.patch("/careers", {...dataUpdated[reorderIndex], timestamp: newData.timestamp});
      ...
      <div className={styles.container}>
        {data.map((el, index) => 
          <div className={`${styles.element} ${reorderIndex === index ? styles.selected : ""}`} key={el._id} onClick={() => onReorder(index)}>
            <h3>{index+1}. {el.message.substring(0, 10)}</h3>
          </div>  
        )}
      </div>
      :
